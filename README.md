...
- **30 tools across the whole TestLink model** — cases, suites, plans, builds, executions,
...
Every tool above works against a stock TestLink with the XML-RPC API enabled, with one
exception: **`update_project` needs `tl.updateTestProject`, a method upstream TestLink
does not ship.** It is available in the
[`dogkeeper886/testlink-code`](https://github.com/dogkeeper886/testlink-code) fork on the
`main` branch. Point `update_project` at a server without it and the tool says so by
name rather than returning a raw XML-RPC fault.
