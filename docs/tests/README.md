...
defines, shared with the `agent-workflows-runner` plugin's `qa-*` commands. The bound `cicd/`
YAML owns **how it runs** (execution): `/qa-bind` links them and `/qa-review-bind` audits the
link — via `npm --prefix cicd/tests` (`audit-bind` / `port-yaml`).
...
The directory is the story; the front-matter `story:` line still records it explicitly, so a
test traces to its intent by grep rather than by path alone. The tooling walks the tree, so a
doc must live under its story directory to be seen at all.
...
- `story` traces the scenario back to its intent; it is **not** hashed. There was a
  `story_hash` drift anchor here, read by a `qw-drift` gate this repo owned — both are gone
  now that the QA lifecycle ships from the `agent-workflows-runner` plugin, which has no
  story-drift command. Nothing detects a story moving underneath its tests; a human does.
