...
function stripApiKey(rows) {
    if (!Array.isArray(rows)) {
        return rows;
    }
    return rows.map(({ api_key, ...rest }) => rest);
}
...
function validateNodeName(value, fieldName) {
    validateNonEmptyString(value, fieldName);
    if (value.length > MAX_NODE_NAME_LENGTH) {
        throw new Error(`${fieldName} must be ${MAX_NODE_NAME_LENGTH} characters or fewer (got ${value.length}). TestLink stores it in a varchar(100) column and rejects longer values without a usable message.`);
    }
}
class TestLinkAPI {
    client;
    ...
    async updateTestCase(testCaseId, data) {
        ...
        if (data.summary)
            updateParams.summary = data.summary;
        if (data.preconditions)
            updateParams.preconditions = data.preconditions;
        if (data.steps)
            updateParams.steps = data.steps;
        if (data.importance !== undefined)
            updateParams.importance = data.importance;
    ...
    async resolveTestProject(projectId) {
        validateNonEmptyString(projectId, 'Project ID/prefix');
        const projects = await this.getTestProjects();
        const rows = Array.isArray(projects) ? projects : [];
        const target = rows.find((p) => String(p.id) === projectId)
            || rows.find((p) => p.prefix === projectId);
        if (!target) {
            throw new Error(`Test project not found: ${projectId}`);
        }
        return target;
    }
    async deleteTestProject(projectId, confirmPrefix) {
        ...
        if (target.prefix !== confirmPrefix) {
            throw new Error(`Confirmation failed: confirm_prefix "${confirmPrefix}" does not match the test case prefix "${target.prefix}" of project "${target.name}". Nothing was deleted.`);
        }
        return this.handleAPICall(() => this.client.deleteTestProject({ prefix: target.prefix }));
    }
    async updateTestProject(projectId, data) {
        if (!data || typeof data !== 'object') {
            throw new Error('Update data must be an object');
        }
        const target = await this.resolveTestProject(projectId);
        ...
        const updateParams = { testprojectid: parseInt(target.id) };
        ...
        if (data.is_public !== undefined)
            updateParams.public = data.is_public;
        ...
        const options = {};
        if (data.requirements_enabled !== undefined)
            options.requirementsEnabled = data.requirements_enabled;
        if (data.test_priority_enabled !== undefined)
            options.testPriorityEnabled = data.test_priority_enabled;
        if (data.automation_enabled !== undefined)
            options.automationEnabled = data.automation_enabled;
        if (data.inventory_enabled !== undefined)
            options.inventoryEnabled = data.inventory_enabled;
        ...
        if (Object.keys(updateParams).length === 1) {
            throw new Error('No fields to update: supply at least one of name, notes, active, is_public, color, requirements_enabled, test_priority_enabled, automation_enabled, inventory_enabled');
        }
    ...
    async callForkMethod(method, params) {
        try {
            return await this.handleAPICall(() => this.client._performRequest(method, params));
        }
        catch (error) {
            if (error.message?.includes(`tl.${method} does not exist`)) {
                throw new Error(`This TestLink server does not provide tl.${method}, which this tool requires. That method ships in the TestLink fork at https://github.com/dogkeeper886/testlink-code (branch main) — see "TestLink server requirements" in the testlink-mcp README.`);
            }
            throw error;
        }
    }
    async getTestSuites(projectId, parentSuiteId) {
    ...
    async createTestSuite(projectId, suiteName, details = '', parentId) {
        validateProjectId(projectId);
    ...
    async updateTestSuite(suiteId, projectId, data) {
        validateSuiteId(suiteId);
        validateProjectId(projectId);
        if (!data || typeof data !== 'object') {
            throw new Error('Update data must be an object');
        ...
        if (data.details)
            updateParams.details = data.details;
    ...
    async getRequirement(requirementId, projectId) {
    ...
    async deleteTestSuite(suiteId) {
        ...
        return this.handleAPICall(() => this.client._performRequest('deleteTestSuite', {
            testsuiteid: parseInt(suiteId)
        }));
    }
    async getRequirementSpecifications(projectId) {
        validateProjectId(projectId);
        return this.handleAPICall(() => this.client._performRequest('getRequirementSpecificationsForTestProject', {
            testprojectid: parseInt(projectId)
        }));
    }
    async createRequirementSpecification(data) {
        if (!data || typeof data !== 'object') {
        ...
        return this.handleAPICall(() => this.client._performRequest('createRequirementSpecification', {
    ...
    async createRequirement(data) {
        ...
        return this.handleAPICall(() => this.client._performRequest('createRequirement', {
            testprojectid: parseInt(data.project_id),
    ...
    async deleteRequirementSpecification(reqSpecId) {
        validateSuiteId(reqSpecId);
        return this.handleAPICall(() => this.client._performRequest('deleteRequirementSpecification', {
            reqspecid: parseInt(reqSpecId)
        }));
    }
    async assignRequirements(data) {
        if (!data || typeof data !== 'object') {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'read_test_case': {
                const result = await testlinkAPI.getTestCase(args.test_case_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'update_test_case': {
                const result = await testlinkAPI.updateTestCase(args.test_case_id, args.data);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            ...
            case 'delete_test_case': {
                const result = await testlinkAPI.deleteTestCase(args.test_case_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_projects': {
            ...
            case 'update_project': {
                const result = await testlinkAPI.updateTestProject(args.project_id, args);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'delete_project': {
                const result = await testlinkAPI.deleteTestProject(args.project_id, args.confirm_prefix);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_test_suites': {
                const result = await testlinkAPI.getTestSuites(args.project_id, args.parent_suite_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_test_cases_in_suite': {
                const result = await testlinkAPI.getTestCasesForTestSuite(args.suite_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'create_test_suite': {
                const result = await testlinkAPI.createTestSuite(args.project_id, args.suite_name, args.details || '', args.parent_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'update_test_suite': {
                const result = await testlinkAPI.updateTestSuite(args.suite_id, args.project_id, args.data);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_test_plans': {
                const result = await testlinkAPI.getTestPlans(args.project_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            ...
            case 'delete_test_plan': {
                const result = await testlinkAPI.deleteTestPlan(args.plan_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'get_test_cases_for_test_plan': {
                const result = await testlinkAPI.getTestCasesForTestPlan(args.plan_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            ...
            case 'list_builds': {
                const result = await testlinkAPI.getBuilds(args.plan_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'create_build': {
            ...
            case 'close_build': {
                const result = await testlinkAPI.closeBuild(args.build_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            ...
            case 'list_requirements': {
                return { content: [{ type: 'text', text: JSON.stringify(await testlinkAPI.getRequirements(args.project_id), null, 2) }] };
            }
            case 'get_requirement': {
                return { content: [{ type: 'text', text: JSON.stringify(await testlinkAPI.getRequirement(args.requirement_id, args.project_id), null, 2) }] };
            }
            case 'delete_test_suite': {
                const result = await testlinkAPI.deleteTestSuite(args.suite_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_requirement_specifications': {
                const result = await testlinkAPI.getRequirementSpecifications(args.project_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            ...
            case 'delete_requirement_specification': {
                const result = await testlinkAPI.deleteRequirementSpecification(args.reqspec_id);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
